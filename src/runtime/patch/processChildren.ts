 ...
 */
export const patchChildren = (oldVNode: VNode, newVNode: VNode, container: RendererElement) => {
 ...
 */
export const mountChildren = (children: VNodeArrayChildren, container: RendererElement) => {
 ...
 */
export const patchArrayChildren = (oldVNode: VNode, newVNode: VNode, container: RendererElement) => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    patch(oldVNode.children[i] as VNode, newChildVNode, container);
  }
  
  if (oldChildrenLen > len) {
    unMountChildren(oldVNode.children.slice(len, oldChildrenLen) as VNode[]);
  } else {
    mountChildren(newVNode.children.slice(len, newChildrenLen) as VNodeChildAtom[], container)
  }
}